...
# ====================== 1. Data loading (following the first Python file's method) ======================
def load_real_data():
    """
    Load real data following the first Python file's method
    """
    ...
    # Temporal split: 85% training, 15% validation (consistent with the first file)
    n_samples = X.shape[0]
...
def prepare_dae_input(X_windows, mode='flatten'):
    ...
    if mode == 'flatten':
        ...
        # Use only the features of the first timestep
        X = X_windows[:, 0, :]
...
# ====================== 3. Deep AutoEncoder construction (following the last file's method) ======================
def build_deep_autoencoder(input_dim, encoding_dim=32):
    """
    Build Deep AutoEncoder model (following the last file's method)
    """
...
# ====================== 5. Evaluation and anomaly detection (consistent with the first file) ======================
...
def visualize_results(history, X_original, X_reconstructed, errors, threshold,
    ...
    # If data is flattened, choose appropriate number of features to display
    ...
    if len(X_original) > 0:
        ...
        if len(X_original.shape) == 2:
            ...
            # If it is a 24-hour * 4 feature structure, reshape to (24, 4)
...
def main():
    ...
    # 1. Load real data (following the first Python file's method)
