...
# ====================== 1. Data loading (following first python file) ======================
def load_real_data():
    """
    Load real data following the method of the first python file
    """
    # Configuration parameters (consistent with the first file)
    ...
    # Time series split: 85% training, 15% validation (consistent with the first file)
    n_samples = X.shape[0]
...
def prepare_dae_input(X_windows_100k, mode='flatten'):
    """
    ...
    """
    n_samples = X_windows_100k.shape[0]
    n_timesteps = X_windows_100k.shape[1]
    n_features = X_windows_100k.shape[2]

    if mode == 'flatten':
        # Flatten to (n_samples, 24*n_features)
        X = X_windows_100k.reshape(n_samples, -1)
...
# ====================== 3. Deep AutoEncoder construction (following last file) ======================
def build_deep_autoencoder(input_dim, encoding_dim=32):
    """
    Build Deep AutoEncoder model (following the method of the last file)
    """
...
# ====================== 5. Evaluation and anomaly detection (consistent with first file) ======================
...
# ====================== 6. Visualization function (following the last file's layout) ======================
def visualize_results(history, X_original, X_reconstructed, errors, threshold,
    ...
    # Set save path
    SAVE_DIR = r"D:\Oswaldo's surf project\DR O's database\visualizations"
    ...
    if len(X_original) > 0:
        ...
        if len(X_original.shape) == 2:
            ...
            # If the structure is 24 hours * 4 features, reshape to (24, 4)
            if feature_errors.shape[0] == 96:  # 24*4=96
                hour_errors = feature_errors.reshape(24, 4)
                ...
                ax8.set_xticklabels([f'Feat_{i}' for i in range(4)], rotation=45, ha='right')
    ...
    save_path = os.path.join(SAVE_DIR, f'deep_ae_visualization100k_data_{timestamp}.png')
...
def main():
    ...
    # 1. Load real data (following the method of the first python file)
    ...
    # 7. Save models
    print("\n💾 Saving models...")
    model_save_path = r"D:\Oswaldo's surf project\DR O's database\models"
    os.makedirs(model_save_path, exist_ok=True)

    # Save in HDF5 format
    autoencoder.save(os.path.join(model_save_path, "deep_ae_autoencoder100k_data.h5"))
    encoder.save(os.path.join(model_save_path, "deep_ae_encoder100k_data.h5"))
    decoder.save(os.path.join(model_save_path, "deep_ae_decoder100k_data.h5"))

    print(f"  Models saved to: {model_save_path}")

    # 8. Generate analysis report (consistent with the first python file)
    print("\n" + "=" * 80)
    ...
    results_path = os.path.join(model_save_path, 'deep_ae_training_results100k_data.json')
