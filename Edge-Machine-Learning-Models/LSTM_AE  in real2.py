...
# Data paths - following the data loading process of the first Python file
...
# Model parameters - following the model structure of the second Python file
TIMESTEPS = 24
LSTM_UNITS = 128
ENCODING_DIM = 32
EPOCHS = 80
BATCH_SIZE = 64  # following the optimization in the second Python file
...
# ====================== 1. Data Loading ======================
def load_data():
    """Following the data loading process of the first Python file"""
    print("=" * 80)
...
def split_data(X, split_ratio=0.85):
    """Following the data splitting method of the first Python file"""
    n_samples = X.shape[0]
...
def build_lstm_autoencoder(sequence_length, n_features):
    """Build LSTM Autoencoder following the model structure of the second Python file"""
...
def train_model(autoencoder, X_train, X_val):
    ...
    # Compile model - using MAE loss function
    autoencoder.compile(
...
def detect_anomalies(autoencoder, X, X_val, threshold_percentile=95):
    """Following the anomaly detection process of the first Python file"""
    ...
    # Full dataset reconstruction errors
...
def visualize_results(df_hourly, mae_full, anomaly_flags, threshold,
    ...
    """Following the visualization style of the second Python file, but only save, not display"""
    ...
    # 1. Training history (if history is provided)
    ...
    # 2. Reconstruction error time series (following the style of the first Python file)
    ...
    if len(anomaly_indices) > 0:
        # Mark anomaly points using integer positions
    ...
    # Set X-axis label based on index type
    if isinstance(time_indices, pd.DatetimeIndex):
        # Set X-axis as window index because timestamps are too dense
    ...
    # 4. Original feature visualization (first 7 days) and 5. Multi-feature comparison plot
    ...
    # 7. Original vs reconstructed comparison (first sample)
...
def main():
    ...
    try:
        ...
        # Call after each model training
        ...
        # 7. Visualization (following the layout of the second Python file, only save not display)
        print("\n" + "=" * 80)
        print("VISUALIZATION")
        print("=" * 80)
        ...
        # 10. Print summary report (following the output format of the first Python file)
        print("\n" + "=" * 80)
