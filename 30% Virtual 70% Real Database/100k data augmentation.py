...
# ==============================================
INPUT_PATH = r"D:\Oswaldo's surf project\Mixed_Real_Virtual_Database_Hourly\preprocessed_data_mixed\normalized_hourly_data.csv"
OUTPUT_PATH = r"D:\Oswaldo's surf project\Mixed_Real_Virtual_Database_Hourly\preprocessed_data_mixed\normalized_hourly_data_augmented_100k.csv"
TARGET_SAMPLES = 100000  # Target samples: 100,000
...
# ==============================================
# 3. Data augmentation to target number (100,000 samples)
# ==============================================
print("\n" + "=" * 60)
print("🚀 Step 2: Augmenting data to 100,000 samples")
print("=" * 60)
...
missing_features = [col for col in scaled_features  if col not in df_complete.columns]
if missing_features:
...
for i in range(multiplier):
    ...
    # Batch shift factor, ensuring each batch has different patterns
    batch_shift = i * 0.5

    # Seasonal factor (12-month cycle)
    season_factor = 1.0 + 0.15 * np.sin(batch_shift + i * np.pi / 6)

    # Yearly trend factor (simulate long-term trend)
    year_trend = 1.0 + 0.05 * np.sin(batch_shift + i * np.pi / 12)

    # 3.4.3 Apply multi-level temporal patterns
    ...
    # Combined temporal pattern noise
    ...
    # First compute daily averages, then expand back to hourly
    ...
    # If length mismatch, adjust to correct length
    ...
    # Ensure data stays within reasonable range (for z-score normalized data)
    ...
    #
    # # Diurnal pattern (usually higher during daytime)
    ...
    # Ensure PM2.5 stays within reasonable range (non-negative, and not exceeding extreme values)
...
print(f"  • Multi-level temporal pattern augmentation (hourly/weekly/monthly)")
...
print(f"  • Data range controlled reasonably")
