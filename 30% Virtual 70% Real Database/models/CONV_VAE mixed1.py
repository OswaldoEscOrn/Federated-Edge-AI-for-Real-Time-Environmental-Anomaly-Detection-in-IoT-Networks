...
# ====================== 1. 配置参数 ======================
DATA_DIR = r"D:\Oswaldo's surf project\Mixed_Real_Virtual_Database_Hourly\preprocessed_data_mixed"
X_PATH = os.path.join(DATA_DIR, "X_windows.npy")
SCALED_DF_PATH = os.path.join(DATA_DIR, "normalized_hourly_data.csv")
MODEL_PATH = r"D:\Oswaldo's surf project\Mixed_Real_Virtual_Database_Hourly\models\conv_vae_multi_modal.h5"
VISUALIZATION_PATH = r"D:\Oswaldo's surf project\Mixed_Real_Virtual_Database_Hourly\visualizations"
...
if history is not None:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(VISUALIZATION_PATH, 'conv_vae_training_history.png'), dpi=150, bbox_inches='tight')
    plt.close()
...
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig(os.path.join(VISUALIZATION_PATH, 'conv_vae_reconstruction_errors.png'), dpi=150, bbox_inches='tight')
plt.close()
...
def create_comprehensive_visualization():
    ...
    if len(FEATURE_NAMES) > 0:
        ...
        if len(anom_idx) > 0:
            ax5.scatter(anom_idx, plot_df.loc[anom_idx, feat],
...
"""
    ...
    save_path = os.path.join(VISUALIZATION_PATH, f"conv_vae_comprehensive_{timestamp}.png")
...
def save_results_vae(results_dict, save_dir):
    ...
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    results_path = os.path.join(save_dir, f"conv_vae_results_{timestamp}.json")
...
# 保存JSON文件
models_dir = r"D:\Oswaldo's surf project\Mixed_Real_Virtual_Database_Hourly\models"
