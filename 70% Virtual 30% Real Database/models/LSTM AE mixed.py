...
# Data path - following the data loading process of the first Python file
...
ENCODING_DIM = 32
EPOCHS = 80
BATCH_SIZE = 64  # Optimized according to the second Python file
...
# ====================== 1. Data Loading ======================
def load_data():
    """Data loading process following the first Python file"""
    print("=" * 80)
...
def split_data(X, split_ratio=0.85):
    """Data splitting method following the first Python file"""
    n_samples = X.shape[0]
...
def train_model(autoencoder, X_train, X_val):
    """Train model, combining the advantages of both Python files"""
    print(f"\n🚀 Starting training...")
...
def detect_anomalies(autoencoder, X, X_val, threshold_percentile=95):
    """Anomaly detection process following the first Python file"""
...
def visualize_results(df_hourly, mae_full, anomaly_flags, threshold,
    ...
    """Visualization style following the second Python file, but not displayed, only saved"""
    ...
    if X_original is not None and X_reconstructed is not None and len(X_original) > 0:
        # Compute feature-level error for the first sample
        sample_idx = 0
...
def main():
    ...
    try:
        ...
        # 4. Train model (combining the advantages of both files)
        print("\n" + "=" * 80)
        print("MODEL TRAINING")
        print("=" * 80)
        history, total_params = train_model(autoencoder, X_train, X_val)

        # Called after each model training
