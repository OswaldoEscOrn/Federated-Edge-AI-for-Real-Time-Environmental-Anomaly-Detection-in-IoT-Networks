...
# ====================== 1. Data Loading (following the first python file) ======================
def load_real_data():
    """
    Load real data following the approach of the first python file
    """
...
def prepare_dae_input(X_windows, mode='flatten'):
    ...
    if mode == 'flatten':
        ...
        # Use only the first time step's features
        X = X_windows[:, 0, :]
...
# ====================== 3. Deep AutoEncoder Construction (following the last file's style) ======================
def build_deep_autoencoder(input_dim, encoding_dim=32):
    """
    Build Deep AutoEncoder model (following the last file's style)
    """
...
def visualize_results(history, X_original, X_reconstructed, errors, threshold,
    ...
    # If data is flattened, select appropriate number of features to display
    ...
    for i in range(n_samples_to_show):
        ...
        if len(X_original.shape) == 2:
            ...
            # Time series data, display first feature
            if i == 0:
    ...
    if len(X_original) > 0:
        ...
        if len(X_original.shape) == 2:
            ...
            # If it's 24 hours * 4 features structure, reshape to (24, 4)
...
def main():
    ...
    # 1. Load real data (following the first python file's approach)
    X_all, X_train, X_val, df_hourly = load_real_data()

    # 2. Prepare training data (flatten windows)
