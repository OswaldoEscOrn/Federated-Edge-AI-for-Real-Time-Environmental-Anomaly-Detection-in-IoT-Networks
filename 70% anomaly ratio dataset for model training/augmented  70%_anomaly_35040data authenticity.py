...
def environmental_augmentation_validation(original_df, augmented_df):
    ...
    print("=" * 60)
    print("Environmental Data Augmentation Quality Validation Report")
    print("=" * 60)
    ...
    # 2. Physical constraints check (most important!)
    print("\n1. Physical constraints check:")

    # PM2.5 cannot be negative
    pm25_negative = augmented_df['avg_PM2.5'] < 0
    if pm25_negative.any():
        print(f"  ⚠ Warning: {pm25_negative.sum()} negative PM2.5 records")
    else:
        print(f"  ✓ All PM2.5 values are non-negative")

    # Scaled features should be within reasonable range
    for feat in scaled_features:
        outliers = augmented_df[feat].abs() > 5  # after standardization, typically within [-3,3]
        if outliers.any():
            print(f"  ⚠ {feat}: {outliers.sum()} values outside ±5 range")

    # 3. Distribution property validation (for different feature types)
    print("\n2. Distribution property validation:")
    ...
    plt.tight_layout()
    plt.savefig(r"D:\Oswaldo's surf project\DR O's database\final_preprocessed_data_70%_anomaly_complete\35040_environmental_dist_comparison.png", dpi=150)
    plt.close()

    # 4. Correlation preservation verification (environmental indicators usually correlate)
    ...
    plt.tight_layout()
    plt.savefig(r"D:\Oswaldo's surf project\DR O's database\final_preprocessed_data_70%_anomaly_complete\35040_augmented_data_correlation.png", dpi=150)
    plt.close()
    ...
    print(f"  Average correlation difference: {mean_diff:.3f}")

    if mean_diff < 0.1:
        print(f"  ✓ Correlation structure preserved well")
    else:
        print(f"  ⚠ Warning: Significant change in variable relationships")

    # 5. Time series pattern check (if temporal info available)
    ...
    print(f"  Proportion of augmented data flagged as anomalous by original data model: {anomaly_rate:.2%}")

    if anomaly_rate < 0.1:
        print(f"  ✓ Augmented data consistent with original data pattern")
    elif anomaly_rate < 0.2:
        print(f"  ⚠ Note: {anomaly_rate:.1%} of augmented data differs from original pattern")
    else:
        print(f"  ⚠ Warning: Large portion ({anomaly_rate:.1%}) of augmented data inconsistent with original pattern")

    # 7. Clustering structure preservation
    print("\n6. Clustering structure check:")
    ...
    # Predict augmented data using the same cluster centers
    aug_labels = kmeans_orig.predict(augmented_df[features].fillna(0))

    # Also cluster augmented data separately for comparison
    ...
    print(f"  Average cluster center difference: {center_diff:.3f}")
    ...
    scores = {
        ...
        'Clustering structure': 1.0 if center_diff < 0.2 else 0.7
    }
    ...
    if overall_score >= 0.9:
        rating = "Excellent - ready to use"
    elif overall_score >= 0.7:
        rating = "Good - recommended to check distributions"
...
def main():
    ...
    augmented_df = pd.read_csv(r"D:\Oswaldo's surf project\DR O's database\final_preprocessed_data_70%_anomaly_complete\preprocessed_time_series_augmented.csv")
    ...
    if 'Unnamed: 0' in original_df.columns:
        print("🔄 Dropping index column from original data...")
        original_df = original_df.drop(columns=['Unnamed: 0'])

    if 'Unnamed: 0' in augmented_df.columns:
        print("🔄 Dropping index column from augmented data...")
    ...
    report_path = r"D:\Oswaldo's surf project\DR O's database\final_preprocessed_data_70%_anomaly_complete\35040_environmental_augmentation_validation_report.txt"

    with open(report_path, 'w', encoding='utf-8') as f:
        f.write("Environmental Data Augmentation Validation Report\n")
        f.write("=" * 50 + "\n\n")
    ...
    if overall_score >= 0.9:
        print("✅ Excellent: Augmentation quality is very high, ready for model training")
    elif overall_score >= 0.7:
        print("✅ Good: Augmentation quality is good, check distribution plots before use")
    elif overall_score >= 0.5:
        print("⚠️ Moderate: Augmentation quality is average, consider regenerating or manual adjustment")
    else:
        print("❌ Needs improvement: Augmentation quality is poor, redesign augmentation strategy")
