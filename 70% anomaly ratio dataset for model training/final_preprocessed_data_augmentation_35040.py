...
# ==============================================
INPUT_PATH = r"D:\Oswaldo's surf project\DR O's database\final_preprocessed_data_70%_anomaly_complete\preprocessed_time_series.csv"
OUTPUT_PATH = r"D:\Oswaldo's surf project\DR O's database\final_preprocessed_data_70%_anomaly_complete\preprocessed_time_series_augmented.csv"
TARGET_SAMPLES = 35064  # 35,040 samples ≈ 4 years of hourly data (365.25*24*4 = 35064, close)

# ==============================================
# 1. Load and fill missing timestamps
# ==============================================
print("=" * 60)
print("🔧 Step 1: Filling missing timestamps")
print("=" * 60)
...
# 1.2 Analyze missing time gaps
print("\n📈 Analyzing time gaps...")

# Create complete hourly time range
...
# 1.3 Reindex to fill missing
print("\n🔧 Filling missing time points...")
df_complete = df.reindex(expected_times)

# Statistics before and after filling
...
if nan_count > 0:
    print(f"  ⚠ Still {nan_count} missing values, using forward fill")
...
if (time_diff == pd.Timedelta(hours=1)).all():
    print(f"  ✅ Time series is completely continuous (hourly)")
else:
    irregular = (time_diff != pd.Timedelta(hours=1)).sum()
    print(f"  ⚠ Found {irregular} irregular time intervals")

# ==============================================
# 3. Data augmentation to target number
# ==============================================
print("\n" + "=" * 60)
print("🚀 Step 2: Augmenting data to 35,040 samples")
print("=" * 60)
...
if missing_features:
    print(f"❌ Missing required feature columns: {missing_features}")
    ...
    for col in missing_features:
        if '_scaled' in col:
            ...
            if alt_col in df_complete.columns:
                ...
                print(f"   Using {alt_col} as substitute for {col}")
...
# 3.3 Calculate augmentation multiplier
...
# Starting time (starting from the beginning of original data)
...
for i in range(multiplier):
    ...
    # 3.4.2 Add seasonal noise
    # Create seasonal pattern based on batch index
    season_factor = 1.0 + 0.15 * np.sin(i * np.pi / 6)  # 6-month cycle

    # Add time-dependent noise
    hour_of_day = np.arange(len(batch_scaled)) % 24
    day_noise = np.sin(hour_of_day * np.pi / 12).reshape(-1, 1) * 0.1

    # Add random noise (preserving z-score characteristics)
    random_noise = np.random.normal(0, 0.05, batch_scaled.shape)

    # Apply noise
    batch_scaled = batch_scaled + day_noise + random_noise * season_factor

    # 3.4.3 Apply seasonal adjustment to PM2.5
    # PM2.5 has obvious seasonal variation, typically higher in winter
    month_factor = 1.0 + 0.2 * np.sin((i % 12) * np.pi / 6)  # 12-month cycle
    batch_pm25 = batch_pm25 * month_factor

    # Add some random variation
    pm25_noise = np.random.normal(0, batch_pm25.std() * 0.05, len(batch_pm25))
    batch_pm25 = np.clip(batch_pm25 + pm25_noise, 0, None)  # PM2.5 cannot be negative

    # 3.4.4 Generate timestamps
    # Shift each batch by 1 year to create continuous time series
    batch_timestamps = pd.date_range(
        start=current_time + pd.DateOffset(years=i),
        periods=len(batch_scaled),
        freq='h'
    )

    # 3.4.5 Add to total data
    all_scaled_data.append(batch_scaled)
    all_pm25_data.append(batch_pm25)
    all_timestamps.extend(batch_timestamps)

    # If target number is reached or exceeded, stop early
    total_so_far = sum(len(batch) for batch in all_scaled_data)
    if total_so_far >= TARGET_SAMPLES:
        print(f"  ⏹ Target samples reached, stopping augmentation")
        break

# 3.5 Concatenate all batches
...
print(f"✅ Data merging completed:")
...
print(f"   Irregular intervals: {irregular_aug}")
...
print("   Original data stats vs Augmented data stats")
...
for col in scaled_features + ['avg_PM2.5']:
    if col in df_complete.columns and col in augmented_df.columns:
        ...
        print(f"      Augmented - mean: {aug_stats['mean']:.4f}, std: {aug_stats['std']:.4f}")

        mean_diff = abs(aug_stats['mean'] - orig_stats['mean']) / orig_stats['mean'] * 100
        std_diff = abs(aug_stats['std'] - orig_stats['std']) / orig_stats['std'] * 100
...
print(f"   Number of NaN values: {nan_count_aug}")
print(f"   Number of infinite values: {inf_count}")

if nan_count_aug == 0 and inf_count == 0:
    print("   ✅ Data is complete, no anomalous values")
else:
    print(f"   ⚠ Found anomalous values, need handling")

# 5.4 Check if target sample count reached
...
# Add statistical comparison info
for col in scaled_features + ['avg_PM2.5']:
    if col in df_complete.columns and col in augmented_df.columns:
        ...
        enhancement_report['statistical_comparison'][col] = {
            ...
            'mean_diff_percentage': float(abs(aug_mean - orig_mean) / orig_mean * 100),
            'std_diff_percentage': float(abs(aug_std - orig_std) / orig_std * 100)
        }
...
print(f"\n📁 Generated files:")
print(f"  1. {OUTPUT_PATH} - Augmented time series data")
print(f"  2. {report_path} - Augmentation processing report")

print(f"\n🔧 Augmentation features:")
print(f"  • Maintained time series continuity (hourly)")
print(f"  • Preserved statistical properties of original data")
print(f"  • Added reasonable seasonal variations")
print(f"  • Maintained z-score normalized feature characteristics")
