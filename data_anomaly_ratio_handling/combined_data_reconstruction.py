...
def create_30percent_anomaly_dataset_time_series(input_path, output_path, target_ratio=0.30):
    """
    Create a dataset with 30% anomaly ratio, preserving time series structure
    Adjust the ratio by resampling anomaly and normal samples while maintaining time order
    """
    print("=" * 60)
    print("Creating dataset with 30% anomaly ratio (preserving time series)")
    print("=" * 60)
...
output_file = r"D:\Oswaldo's surf project\DR O's database\data_anomaly_ratio_handling\reconstruction_data.csv"
...
result_df = create_30percent_anomaly_dataset_time_series(
