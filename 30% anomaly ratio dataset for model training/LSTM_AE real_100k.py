...
# Model save path - updated to new path
MODEL_SAVE_DIR = r"D:\Oswaldo's surf project\DR O's database\final_preprocessed_data_complete\models"
os.makedirs(MODEL_SAVE_DIR, exist_ok=True)
MODEL_PATH = os.path.join(MODEL_SAVE_DIR, "100k_lstm_autoencoder_hybrid.h5")

# Visualization save path - updated to new path
...
# Feature names - adjust according to actual data
FEATURE_NAMES = [
...
def visualize_results(df_hourly, mae_full, anomaly_flags, threshold,
    ...
    save_path = os.path.join(VISUALIZATION_SAVE_DIR, f"100k_lstm_ae_visualization_{timestamp}.png")
...
def save_results(results_dict, save_dir):
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    results_path = os.path.join(save_dir, f"100k_lstm_ae_results_{timestamp}.json")
...
def main():
    ...
    try:
        ...
        encoder_path = os.path.join(MODEL_SAVE_DIR, "100k_lstm_ae_encoder.h5")
